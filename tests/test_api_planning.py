...
async def test_planning_flow(client, seeded, token_admin, token_member, token_leader):
    ...
    r = await client.post(
        f"/api/planning/proposals/{proposal['id']}/accept", headers=token_leader
    )
    ...
    r = await client.post(
        f"/api/planning/proposals/{proposal['id']}/decline", headers=token_admin
    )
...
async def test_planning_withdraw_rights(
    ...
    r = await client.post(
        f"/api/planning/proposals/{pid}/withdraw", headers=token_member
    )
    assert r.status_code == 403

    # the proposer may withdraw their own proposal
    r = await client.post(
        f"/api/planning/proposals/{pid}/withdraw", headers=token_leader
    )
    assert r.status_code == 200
