"""The host-side sheet-decoration script (deploy/files/, runs outside the
container with no venv). Tested here: the constants that must track the app
(role labels, column positions), the pure request builder and its safety
property (field masks that cannot reach cell values), state distillation,
rclone-conf parsing, and SERVICE_DISABLED detection. NOT tested, mirroring
test_drive_sync.py: the live token mint, the Sheets API itself, and the
rclone listing — those are covered by the runbook in
...
def _spreadsheet(note=None, labels=None, protections=None, row_count=40) -> dict:
    ...
    validation = {
        "condition": {
            "type": "ONE_OF_LIST",
            "values": [{"userEnteredValue": label} for label in labels],
        },
        "strict": True,
        "showCustomUi": True,
    }
                ...
                "data": [
                    {
                        "rowData": [
                            ...
                            {"values": [{}] * 7 + [{"dataValidation": validation}]},
                        ],
...
def test_build_requests_golden():
    ...
    protection = requests[4]["addProtectedRange"]["protectedRange"]
