...
async def fields_page():
    ...
    with frame("Custom fields", actor):
        ...
        ui.button("New field", icon="add", on_click=lambda: _field_dialog()).props("dense")
        ...
        for defn in defs:
            with ui.row().classes("w-full items-center gap-3 p-2 rounded hover:bg-gray-100"):
                ...
                ui.space()
                ui.button(icon="edit", on_click=lambda _, d=defn: _field_dialog(d)).props(
                    "dense flat"
                )
                ui.button(icon="delete", on_click=lambda _, d=defn: _delete_dialog(d)).props(
...
def _field_dialog(defn=None) -> None:
    ...
    with ui.dialog() as dialog, ui.card().classes("w-96 gap-3"):
        ...
        label = ui.input("Label", value=defn.label if defn else "").props(
            "outlined dense"
        ).classes("w-full")
        if defn is None:
            field_type = ui.select(TYPE_OPTIONS, label="Type", value=FieldType.text.value).props(
                "outlined dense"
            ).classes("w-full")
        else:
            field_type = None
        ...
        if defn is None:
            options.bind_visibility_from(field_type, "value", lambda v: v == FieldType.select.value)
        else:
            options.set_visibility(defn.field_type == FieldType.select.value)
        show_in_list = ui.switch(
            "Show as a column on the volunteers list", value=defn.show_in_list if defn else False
        )
        position = ui.number("Sort position", value=defn.position if defn else 0, precision=0).props(
            "outlined dense"
        ).classes("w-32")
        ...
        @notify_errors
        async def save() -> None:
            option_list = [line for line in (options.value or "").splitlines() if line.strip()]
