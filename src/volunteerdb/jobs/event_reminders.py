...
The job is read -> plan -> execute: `read` pulls plain rows, `plan` (pure,
tested without a database) decides who gets which digest and which stages
are stamped, and `execute` sends through the Env's mailer and stamps each
person in a transaction of their own.
...
class ReminderRow:
    """One assignment still owed a notice, as plain values."""
...
type Stamp = tuple[int, NotificationStage]  # (assignment id, stage)


@dataclass(frozen=True)
class Digest:
    """One person's mail for the night, and the stages it settles."""
...
async def read(
...
) -> tuple[list[ReminderRow], dict[int, str], frozenset[Stamp]]:
    """Every assignment on a live event that may still be owed a notice, the
    team paths, and the stages already recorded for those assignments."""
    rows = (
    ...
    plain = [
        ReminderRow(
            ...
            notify_7d=assignment.notify_7d,
            notify_24h=assignment.notify_24h,
...
def plan(
    ...
    *,
    ...
    """Who gets a digest tonight, listing what, and which stages that settles.
    Pure: the windows are decided from `today` and the event's parish day."""
    per_person: dict[int, tuple[str, list[mail.EventDigestItem], list[Stamp]]] = {}
    for row in rows:
        event_day = row.starts_at.astimezone(tz).date()
        days_out = (event_day - today).days

        def sent(stage: NotificationStage, _id: int = row.assignment_id) -> bool:
        ...
        items.append(
            mail.EventDigestItem(
                kind=kind,
                title=row.title,
                path=paths.get(row.team_id, f"team {row.team_id}"),
                slot=row.slot,
        ...
        if pending_scheduled:
            stamps.append((row.assignment_id, NotificationStage.event_scheduled))
        if not sent(NotificationStage.event_week) and days_out <= WEEK_DAYS:
            stamps.append((row.assignment_id, NotificationStage.event_week))
...
async def main(env: Env, today: date | None = None) -> int:
    ...
    if today is None:
        today = env.today()
    async with transaction(env, None) as session:
        rows, paths, already = await read(session, now=env.clock.now())
    report = await execute(plan(rows, paths, already, today=today, tz=env.tz), env)
    print(
        f"event reminders: {report.sent} person(s) emailed, {report.failed} failure(s)"
    )
    return 0
